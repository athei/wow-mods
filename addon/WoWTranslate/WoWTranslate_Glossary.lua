...
WoWTranslateGlossary["黑色沼泽"]           = "The Black Morass"
...
-- Length-sorted glossary keys, built once on first use. The glossary is
-- immutable at runtime, so collecting and sorting its ~1700 keys on every
-- partial-match call (i.e. every untranslated incoming message) was pure waste.
local sortedGlossaryKeys = nil
local function GetSortedGlossaryKeys()
    if not sortedGlossaryKeys then
        sortedGlossaryKeys = {}
        for k, _ in pairs(WoWTranslateGlossary) do
            table.insert(sortedGlossaryKeys, k)
        end
        table.sort(sortedGlossaryKeys, function(a, b) return string.len(a) > string.len(b) end)
    end
...
function WoWTranslate_CheckGlossaryPartial(text)
    local sortedKeys = GetSortedGlossaryKeys()
