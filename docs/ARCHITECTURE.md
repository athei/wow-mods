...
Numeric convention: kernels compute in `f32`, matching the SSE lane width, unless a function has to track a value the original kept in an x87 register across operations — then it computes in `f64` and narrows through `f64_to_f32`. Every float-to-int conversion routes through `wow_shared::ftol`, because a compiler-lowered `f64 as i64` on i686 re-emits its own control-word dance and rounds differently from what the client did.

The precision that convention rests on is **53-bit, not 80-bit**, and it is worth stating because the wrong reading inverts the rule. The client sets it itself: `_controlfp(0x9001F, -1)` at `0x404105`, whose `_PC_53` selector puts the x87 control word at `0x023F` — an absolute set, not a merge. Nothing overrides it afterwards. Every `fldcw` in the image is CRT-internal and save/restore-balanced, and the D3D device is created with BehaviorFlags `0x22`/`0x52`, both carrying `D3DCREATE_FPU_PRESERVE`, so the graphics layer never forces the 24-bit mode it otherwise would.

So an x87 register holds an `f64` significand, and `f64`-and-narrow-once is **bit-exact rather than approximate** — within the normal range only, since x87 keeps a 15-bit exponent and the equivalence does not extend to overflow or denormals. The corollary is the one that catches people: per-step `f32` is correct *only* where the original genuinely stores each intermediate to an `f32` slot. A chain the original left in a register wants `f64`, and a chain it spilled between every operation wants `f32`. Read the stores, not the operand widths.
...
2. **Wine resolves builtins by name through the prefix, not through `lib/wine`.** Without a placeholder named `wow_mods.dll` in the prefix's `syswow64` the real builtin is never found. `wineboot` writes one for every builtin it finds in `lib/wine` when it creates a prefix (the `11,,*` wildcard at the end of `wine.inf`'s `[FakeDllsWow64]`), custom names included, so an install that lands before the prefix needs nothing. For a prefix that came first, `make windows` runs `winebuild --fake-module` to produce `wow_mods.fake.dll`, which `bundle` ships under `prefix-markers/` for the user to place. This is the single most likely thing to break a hand install.
